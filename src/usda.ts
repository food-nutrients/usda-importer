...
export class USDA {
  ...
  public async cachedGetFoodById(foodId: string, accessToken: string): Promise<UsdaFood> {
    ...
    try {
      ...
      food = await this.getFoodByID(foodId, accessToken);
  ...
  private findNutrient(nutrients: UsdaFoodNutrient[], nutrientId: number): UsdaFoodNutrient {
    return nutrients.find((nutrient: UsdaFoodNutrient) => nutrient.nutrient_id === nutrientId);
  }

  private extractNutrient(nutrients: UsdaFoodNutrient[]): (nutrientId: number) => number {
    return (nutrientId: number) => {
      const nutrient = this.findNutrient(nutrients, nutrientId);
